...
export default function addRegA_Actividad({idDepto, idEst}) {
  ...
  return (
    <ScrollView centerContent={true} style={styles.viewBody} >
      ...
      <TouchableOpacity onPress = {() => navigation.navigate('add-FormularioEst',{idActiv:item.id, idPers:item.idPers,idEst:idEst})}>
      <ListItem key={i} bottomDivider>
          ...
          <ListItem.Title>{item.horarioAct}</ListItem.Title>
          <ListItem.Title>{item.periodoAct}</ListItem.Title>
          </ListItem.Content>
          </ListItem>
        </TouchableOpacity>
      ))}
